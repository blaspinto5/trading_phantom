...
def bot_start():
    ...
    with _bot_lock:
        ...
        t = threading.Thread(
            target=_forward_output_to_log, args=(proc.stdout, bot_log), daemon=True
        )
        t.start()

        _bot_process = proc
        logger.info(
            "Started bot subprocess pid=%s; logging to %s", _bot_process.pid, bot_log
        )
...
def shutdown():
    ...
    def _do_shutdown():
        ...
        # Cerrar BD si existe
        try:
            from trading_phantom.analytics.db import engine, get_db_session
