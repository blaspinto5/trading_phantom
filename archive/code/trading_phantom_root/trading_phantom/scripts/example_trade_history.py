...
def example_usage():
    ...
    history.close_trade(
        ticket=100001, exit_price=1.16790, profit_loss=290.00  # 29 pips × 10 USD = 290
    )

    history.close_trade(
        ticket=100002, exit_price=1.16410, profit_loss=340.00  # 34 pips × 10 USD = 340
    )

    history.close_trade(
        ticket=100003, exit_price=1.15950, profit_loss=-250.00  # Pérdida de 50 pips
    )
    ...
    for trade in recent:
        ...
        print(
            f"   Ticket {trade['ticket']}: {trade['signal']:4} | {trade['symbol']:7} | {status}"
        )
