...
class BacktestEngine:
    ...
    def generate_simulated_trades(self, num_trades=200):
        ...
        for i in range(num_trades):
            ...
            profit_loss = np.random.normal(50, 100) if np.random.random() < win_prob else np.random.normal(-50, 100)
    ...
    def calculate_metrics(self):
        ...
        max_profit = df[df['profit_loss'] > 0]['profit_loss'].max() if (df['profit_loss'] > 0).any() else 0
        max_loss = df[df['profit_loss'] < 0]['profit_loss'].min() if (df['profit_loss'] < 0).any() else 0
        
        # Advanced metrics
        profit_factor = abs(df[df['profit_loss'] > 0]['profit_loss'].sum() / df[df['profit_loss'] < 0]['profit_loss'].sum()) if (df['profit_loss'] < 0).sum() > 0 else float('inf')
    ...
    def print_report(self, metrics):
        ...
        # Status check
        status = "🚀 EXCELENTE" if metrics['roi'] > 10 else "✅ BUENO" if metrics['roi'] > 5 else "⚠️ NEUTRAL" if metrics['roi'] > 0 else "❌ REVISAR"
