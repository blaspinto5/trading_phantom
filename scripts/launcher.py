...
def _start_flask(host, port, debug):
    ...
    log.info('Starting Flask server on %s:%s (debug reloader disabled in threaded mode)', host, port)
...
def main():
    ...
    t = threading.Thread(target=_start_flask, args=(args.host, args.port, args.debug), daemon=True)
    t.start()
...
if __name__ == '__main__':
    try:
        main()
    except Exception as exc:  # Capture any unhandled exception and make debugging easier
        import traceback
        import tempfile
        import os
        try:
            ...
            print('Unhandled exception in launcher; traceback written to:', tmp, file=sys.stderr)
