...
class KnowledgeBase:
    ...
    def save_feature_embeddings(self, model, X_train: np.ndarray, 
    ...
    def save_correlation_matrix(self, X_train: np.ndarray, 
                               feature_names: List[str]) -> Dict:
        """
    ...
    def save_decision_patterns(self, model, feature_names: List[str]) -> Dict:
        ...
        data = {
            ...
            'patterns_by_feature': {
                feat: len(pats)
                for feat, pats in patterns_by_feature.items()
            },
    ...
    def save_performance_metrics(self, model, X_val: np.ndarray, y_val: np.ndarray,
    ...
    def save_training_data_stats(self, X_train: np.ndarray, y_train: np.ndarray,
                                feature_names: List[str], class_names: List[str]) -> Dict:
        """
    ...
    def save_trade_patterns(self, trades_history: List[Dict]) -> Dict:
        ...
        patterns = {
            ...
            'avg_win_pnl': float(np.mean([t.get('pnl', 0) for t in winning_trades])) if winning_trades else 0,
            'avg_loss_pnl': float(np.mean([t.get('pnl', 0) for t in losing_trades])) if losing_trades else 0,
